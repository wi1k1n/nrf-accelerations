...
@register_traj('drunk')
def drunk(radius=3.5, h=0.0, N=4, A=2.0, axis='z', t0=0, r=1, o=(0,0,0)):
    assert axis == 'z', 'not Z aligned axis is not supported'
    ra = lambda t: t * np.pi / 180.0
    # o - origin point
    return lambda t: [radius * np.cos(r * t+ra(t0)) + o[0], radius * np.sin(r * t+ra(t0)) + o[1], h + o[2] + A * np.sin(N * t + ra(t0))]
